...
class ElasticityFirstPredictor(nn.Module):
    ...
    def __init__(
        ...
        if x0 is not None:
            # buffer to travel with .to(device)
    ...
    def forward(
        ...
        """
        Args:
            x: (B, n) log-prices target
            c: (B, d) context or None
            x0: (n,) or (B, n) log-prices of reference. If None, uses self.x0.
            order: integration order (permutation of [0..n-1]). If None, uses default_order or natural.
            return_E_at_x: if True, returns also E(x,c) evaluated at the endpoint.

        Returns:
            y_hat: (B, n) or (y_hat, E_x)
        """
        B, n = x.shape
        ...
        if x0 is None:
            if self.x0 is None:
                raise ValueError("x0 not provided and self.x0 is None. Pass x0 or fix x0 in init().")
            x0_use = self.x0
        else:
            x0_use = x0
        ...
        if x0_use.ndim == 1:
            if x0_use.shape[0] != n:
                raise ValueError(f"x0 (vector) must have shape ({n},)")
            x0_batch = x0_use.unsqueeze(0).expand(B, -1)
        elif x0_use.ndim == 2:
            if x0_use.shape != (B, n):
                raise ValueError(f"x0 (batch) must have shape {(B, n)}")
            x0_batch = x0_use
        else:
            raise ValueError("x0 must be (n,) or (B,n)")
